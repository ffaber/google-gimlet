 ...
 * of scopes for the current thead. The head (beginning) of the deck is the most
 * recently added ScopeId.
 */
class ScopeIdStackProvider implements Provider<Iterable<ScopeId>> {
  ...
  @Override public Iterable<ScopeId> get() {
    ...
    for (BindingFrame bindingFrame : bindingFrameStack.getBindingFrames()) {
      ScopeId scopeId =
          checkNotNull(
              bindingFrame.get(Key.get(ScopeId.class, ScopeIdKey.class)),
